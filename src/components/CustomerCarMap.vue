<template>
  <div id="map_wrapper">
    <l-map
        id="map"
        :zoom="zoom"
        :center="center"
        @click="() => {if(selectedCar) {selectedCar = null}}"
    >
      <l-tile-layer
          :url="url"
          :attribution="attribution"
      />
      <template v-if="cars">
        <l-marker v-for="car in cars" :key="car.id" :lat-lng="L.latLng(car.latitude, car.longitude)"
                  @click="showPopup(car)"/>
      </template>
    </l-map>
    <car-viewer id="viewer" v-if="selectedCar" :car="selectedCar"/>
  </div>
</template>

<script lang="ts">
import {Component, Vue} from 'vue-property-decorator';
import L from "leaflet"
import {LMap, LMarker, LPopup, LTileLayer, LTooltip} from "vue2-leaflet";
import "leaflet/dist/leaflet.css"
import {TitleService} from "@/services/TitleService";
import {CarModel} from "@/models/CarModel";
import {CarService} from "@/services/CarService";
import SnackbarService from "@/services/SnackbarService";
import CarViewer from "@/components/CarViewer.vue";

//Workaround
const defaultMarker = 'data:image/png;base64,';
const defaultMarkerShadow = 'data:image/png;base64,';
(L.Icon.prototype as any)._createImg = (src: string, el: HTMLImageElement) => {
  el = el || document.createElement('img');
  if (src.includes('icon')) {
    src = defaultMarker;
  }
  if (src.includes('shadow')) {
    src = defaultMarkerShadow;
  }
  el.src = src;
  return el;
};

@Component({
  components: {CarViewer, LMap, LTileLayer, LMarker, LPopup, LTooltip}
})
export default class CustomerCarMap extends Vue {
  L = L;
  zoom = 12;
  center = L.latLng(51.66, 39.20);
  url = 'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
  attribution = '&copy; <a href="http://osm.org/copyright">OpenStreetMap</a> contributors';
  cars: CarModel[] | null = null;
  selectedCar: CarModel | null = null;

  created(): void {
    TitleService.setTitle("Машины на карте");
    CarService.getAllCars().then(cars => {
      this.cars = cars;
    }).catch(errors => {
      SnackbarService.showErrorSnackbarAndLog("Приоизошла ошибка при загрузке данных", errors);
    });
  }

  showPopup(car: CarModel): void {
    this.selectedCar = car;
  }


}
</script>

<style scoped>
#map_wrapper {
  position: absolute;
  top: 0;
  bottom: 0;
  left: 0;
  right: 0;
}

#map {
  height: 100vh;
  width: 100vw;
  z-index: 0;
}

#viewer {
  z-index: 2;
  position: absolute;
  top: 0;
}
</style>

<style>
.leaflet-control-zoom {
  position: fixed;
  top: 60px;
}
</style>
